...
//			Physics2D.IgnoreLayerCollision(birdsLayer, blocksLayer, true);
	...
	public GameObject AddPig(GameObject original, Vector3 position, Quaternion rotation, float scale = 1f) {
		...
		pig.GetComponent<Rigidbody2D>().simulated = simulateRigidbody;
		if(pig != null)
			_pigs.Add(pig);
	...
	public GameObject AddBlock(GameObject original, Vector3 position, Quaternion rotation, float scaleX = 1f, float scaleY = 1f) {
		...
		newGameObject.GetComponent<Rigidbody2D>().simulated = simulateRigidbody;
	...
	public void SimulateRigidbody(bool isSimulated)
	{
		List<GameObject> objs = BlocksInScene();
		foreach (GameObject o in objs)
		{
			try
			{
				o.GetComponent<Rigidbody2D>().simulated = isSimulated;
			}
			catch { }
		}
		simulateRigidbody = isSimulated;
	}
}
