...
namespace ICE
{
    public static class Grid2LevelConverter
    {
        ...
        private static GridDataset GrDataset
        {
            get
            {
                ...
                {
                    m_gridDataset = Resources.Load<GridDataset>("GridDataset");
                }
        ...
        private static Vector2 GetObjectPosition(int cellId, string blockName, float blockRotation, List<BlockData> currentBlocks)
        {
            try
            {
                Vector2 tarSize = LevelConverter.GetSize(blockName, blockRotation);
