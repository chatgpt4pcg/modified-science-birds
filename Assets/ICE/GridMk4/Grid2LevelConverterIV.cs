...
namespace ICE
{
    public static class Grid2LevelConverterIV
    {
        ...
        private static GridDataset GrDataset
        {
            get
            {
                ...
                {
                    m_gridDataset = Resources.Load<GridDataset>(GRID_RESOURCE_PATH);
                }
                return m_gridDataset;
            }
        }

        private const string GRID_RESOURCE_PATH = "GridDatasetIV";
        private const string FUNCTION_TAG = "drop(";
        ...
        private static Vector2 GetObjectPosition(int cellId, string blockName, float blockRotation, List<BlockData> currentBlocks)
        {
            try
            {
                ...
                val.x = CellLeft(cellId) + tHalfSize.x;
        ...
        private static float GetYPos(string blockName, float blockRotation, int targetCell, List<BlockData> currentBlocks)
        {
            ...
            Rect rayRect = Rect.MinMaxRect(cellLeft, 0,
                cellLeft + (tHalfSize.x * 2.0f), 9999);
